...
// plus synastry/composite/transit variants.
// Examples:
//  - Pp-SusSc12 -> Sun in Scorpio, 12th house
//  - A-MesSa01CaSqJusVi10 -> Mercury in Sagittarius (1st) close square Jupiter in Virgo (10th)
//  - SynA-P1(SusAr03)-CaCo-P2(MesAr04) -> P1's Sun in Aries (3rd) close conjunction P2's Mercury in Aries (4th)
//  - SynP-Moon-H08 -> Moon in 8th house (synastry placement)
//  - CompP-Mercury-H07 -> Composite Mercury in 7th house
...
export type SynastryPlacement = {
  type: 'synastryPlacement';
  planet: string;
  house: number;
  pretty: string;
};
...
const synastryPlacementRe = /^SynP-([A-Za-z]+)-H(\d{2})$/;
...
export function decodeAstroCode(
  ...
  const synPMatch = synastryPlacementRe.exec(code);
  if (synPMatch) {
    const [, planetName, hStr] = synPMatch;
    const house = parseInt(hStr, 10);
    return {
      type: 'synastryPlacement',
      planet: planetName,
      house,
      pretty: `${planetName} in ${houseLabel(house)} house`,
    };
  }
